	...
	\fbox{\begin{minipage}{\dimexpr \textwidth-2\fboxsep-2\fboxrule}
			\begin{equation*}
				\begin{aligned}
					...
					& Network ID contained in bits 1-8 \\
					& \therefore \text{Network ID} = 00000001 = 1 \\
					& Host ID contained in bits 9-32 \\
	...
	\fbox{\begin{minipage}{\dimexpr \textwidth-2\fboxsep-2\fboxrule}
			\begin{equation*}
				\begin{aligned}
					...
					& Network ID contained in bits 3-24 \\
					& \therefore \text{Network ID} = 00001100 00011000 110111 = 12.24.220 \\
					& Host ID contained in bits 25-32 \\
					& \therefore \text{Host ID} = 01111111 = 127 \\
				\end{aligned}
			\end{equation*}
	\end{minipage}}
	\fbox{\begin{minipage}{\dimexpr \textwidth-2\fboxsep-2\fboxrule}
...
As shown in the examples above, when determining the network and host IDs, it is important to:
\begin{itemize}
	\item re-split the result in to groups of 8 bits;
	\item backfill where necessary to ensure that each group is 8 bits; and
	\item maintain the correct place values when backfilling.
\end{itemize}
